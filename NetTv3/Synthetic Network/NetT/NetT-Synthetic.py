...
class SyntheticDataset(DGLDataset):
    ...
    def process(self):
        ...
        nodes_data = pd.read_csv('C:/Users/Frank/Desktop/Data_Set/Main/nodes-syn.csv')
        print ('')
        print(nodes_data)

        edges_data = pd.read_csv('C:/Users/Frank/Desktop/Data_Set/Main/edges-syn.csv')
        print ('')
        print(edges_data)
        print ('')
...
G = dgl.add_self_loop(G) # allows for users to include their features in aggregate of the fearures of neighbor users
...
print ('User 3 has an in-degree of: %d' % G.in_degrees(3))
print ('User 12 has an in-degree of: %d' % G.in_degrees(12))
print ('User 77 has an in-degree of: %d' % G.in_degrees(77))
...
def new_func():
    node_embed = nn.Embedding(80, 8)  # Every node has an embedding of size 8.
    return node_embed
...
for e in range(100):
      ...
      # Compute loss of the nodes in the training set.
      ...
      if e % 5 == 0:
          print('Epoch {}, Loss: {:.3f}, Train Acc: {:.3f}, Val Acc: {:.3f} (best {:.3f}), Test Acc: {:.3f} (best {:.3f})'.format(
...
view_model(99)  # draw the prediction of the epoch numbered - this can be changed
plt.show()
